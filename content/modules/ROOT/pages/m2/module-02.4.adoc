...
. In the *lower terminal*, run the following canary-rollout script. This script shifts traffic form `v1` to `v2` by updating the VirtualService you created earlier,
+
[source,sh,role="execute",subs=attributes+]
----
...
Run the following command on the *upper terminal* to reset traffic to discounts service `v1`
