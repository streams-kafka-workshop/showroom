...
In this section, you'll apply an Istio `AuthorizationPolicy` to enforce a least privilege model, allowing access to `discounts` only from the allowed backend services (`hotels`, `cars`, `flights` and `insurnace` services), and not from the `travels` REST API.


== Review current access of *discounts* service

. Run the following command in the *upper teminal* to verify that you are indeed able to access the `discounts` service directly from all the services in the `travel-agency` name space. +
...
The `discount-access-sa` service account has already created and added to the allowed backend services as part of the workshop set up.
====
...
```
+
WARNING: It maybe take a few seconds for the AuthorizationPolicy to take effect. Please retry if you still see a success response. 
