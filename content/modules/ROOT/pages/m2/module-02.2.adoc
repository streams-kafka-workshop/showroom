...
. In the file explorer on the left side, expand the `module-kafka-api` folder and then expand the `src/main/java/kafka/tutorial` folder. Click on the `KafkaConfig.java` file to open it.

. Replace the bootstrap servers value with the following value.
+
[source,bash,role="execute",subs=attributes+]
----
...
+
image::m2/kafka-config-java-bootstrap-servers.png[]
...
. The next option is *linger.ms.* The default value for our application is 0, which means that the producer will not wait for any messages to accumulate before sending them. 

. Enter `30000` and press enter. This will make the producer wait for 30 seconds until the batch is full before sending it. The producer will send the batch after 30 seconds, even if it is not full. Enter the below value and press enter.
+
[source,bash,role="execute",subs=attributes+]
----
30000
...
. Next, press *enter* to go the menu
