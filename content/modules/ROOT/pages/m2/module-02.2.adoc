...
* Make sure you are using java 17 by running the following command.
+
...
* Press enter to accept the default values, which should trigger the producer sending the messages.
...
* This might be useful setting as it prevents typos in producer or consumer codefrom creating junk topics. 
...
* Run the interactive application again. You should see the following output.
+
[source,bash,role="execute",subs=attributes+]
----
./run.sh
...
* Choose the option 1 by entering `1` and press enter to run the producer.
...
* The next option is linger.ms. The default value for our application is 0, which means that the producer will not wait for any messages to accumulate before sending them. Enter `30000` and press enter. This will make the producer wait for 30 seconds until the batch is full before sending it. The producer will send the batch after 30 seconds, even if it is not full. Enter the below value and press enter.
+
[source,bash,role="execute",subs=attributes+]
----
30000
...
* You should see that some messages are none of the messages are sent immediately. Only after 30 seconds the messages are sent to the topic.
...
* Next, press enter and press `1` to run the producer again.

* Accept the default value for batch size(16 KB) and linger.ms (0) seconds. Before you press enter, take a guess on what will happen and observe the output.
...
* Hit enter and press `3` and press enter again to exit the application.

=== Summary
This section hopefully helped you get a high level understanding of the producer api and see a couple of producer configurations in action. In the next section, we will first learn about the consumer configuration and then see the consumer in action.
