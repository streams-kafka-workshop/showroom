...
NOTE: The rest of this section deals with explaining the various configuration options available for Kafka consumers and some of their usages in the code. If you are already familiar with these options or want to directly jump to running the code,, feel free to skip to this section and move on to the xref:module-02.4.html[next section] to run the consumer application.
