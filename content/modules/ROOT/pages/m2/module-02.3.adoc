...
.DestinationRule
****
A `DestinationRule` defines policies applied to traffic intended for a service after routing has occurred. One of the primary use cases for a DestinationRule is to define *subsets* — logical versions of a service that can be targeted individually. These subsets are typically defined using labels corresponding to different service versions (for example, v1, v2, v3).
****
