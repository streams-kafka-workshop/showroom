...
. Let's us now look at the consumer code. In the `module-kafka-api` folder, click on the *src/main/java/com/example/SimpleKafkaConsumer.java* file in the file explorer to open it.
+
...
* The configuration is defined using `Properties` and constants from `ConsumerConfig`. Look at lines 47-82 of the code.
...
=== Up Next
Now that we have analyzed the Consumer APIs and the related code, let's put running our simple application to the see the consumer in action.