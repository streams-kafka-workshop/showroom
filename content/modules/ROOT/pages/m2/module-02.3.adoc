...
. Let us now look at the consumer code. In the `module-kafka-api` folder, click on the *src/main/java/com/kafka/tutorial/service/ConsumerService.java* file in the file explorer to open it.
+
image::m2/module-kafka-api-consumer_new.png[]

This section breaks down the code and explains how a **Kafka consumer** is configured and how it works under the hood.
...
The **Used in example?** column shows whether the option appears in the code. The values for the the options are provided via the application.properties or environment variable and can be changed by the user when running the application.

[IMPORTANT]
====
The rest of this section deals with explaining the various configuration options available for Kafka consumers and some of their usages in the code. If you are already familiar with these options or want to directly jump to running the code, feel free to **skip to this section and move on to the link:module-02.4.html[next section]** to run the consumer application.
====
