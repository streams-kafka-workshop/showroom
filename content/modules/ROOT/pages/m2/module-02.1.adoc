...
. Navigate to the {ocp_cluster_openshift_cluster_console_url}[OpenShift Console, window="console"] and Login as *admin/{ocp_cluster_openshift_cluster_admin_password}* if prompted.

. You can also run the following command from the terminal window to get the URL in either terminal window.
...
. You can click the link above to access the Kiali console, but if needed, to get the URL to the Kiali console via the command line, run the following command in either terminal window on the right:
+
[source,sh,role="execute",subs=attributes+]
----
