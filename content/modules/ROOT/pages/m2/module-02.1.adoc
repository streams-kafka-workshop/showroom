...
| `linger.ms`
| Time to wait for more records before send
| `0`–`50+`
...
| Compression reduces bandwidth and storage; adds CPU. Great with batching.
| {cross}

| `buffer.memory`
| Total memory for record accumulation
| e.g., `33554432` (32 MiB)
| If full, `send()` may block per `max.block.ms` or throw.
| {cross}
|===
...
| `request.timeout.ms`
| Broker response timeout per request
| e.g., `30000`
| Should be less than `delivery.timeout.ms`.
| {cross}
...
| e.g., `300000`
| Controls how often producer refreshes cluster/topology info.
| {cross}
|===
