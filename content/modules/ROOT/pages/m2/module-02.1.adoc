...
In this module, you will use the Kiali consle to enable automatic Istio injection to the namespace. Service mesh will then automatically inject proxy sidecar containers into all pods in `travel-agency` namespace that will allow the service mesh to  manage the services in the namespace.
...
. In the *lower terminal*, run the following command to restart the pods in `travel-agency` namespace.
+
[source,sh,role="execute",subs=attributes+]
----
...
deployment.apps/discounts-v2 restarted
