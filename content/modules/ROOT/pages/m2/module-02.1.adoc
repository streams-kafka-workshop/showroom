...
The Kafka producer API allows developers to write kafka producers within their applications to send records to Kafka topics. The API provides various configuration options to control the behavior of the producer, such as batching, compression, retries, and acknowledgments.
...
* Navigate to {openshift_cluster_console}/k8s/cluster/projects[OpenShift Cluster Console, window="_console"]. Log in with your username and password (`{user_name}`/`{password}`).
...
* Log in using your credentials  - `{user_name}` and `{password}`
...
* Look at the dependencies section of the *pom.xml file (line 50-55).* You will see that the project has a dependency on the Kafka client library. This library provides the necessary classes and methods to interact with a Kafka cluster.
+
...
This section breaks down the code and explains how a **Kafka Producer** is configured and how it works under the hood.

* Let us now look at the producer code. In the `module-kafka-api` folder, click on the *src/main/java/com/kafka/tutorial/service/ProducerService.java* file in the file explorer to open it.
+
image::m2/producer-service-nav.png[]

In this example:

.  The producer connects to a Kafka cluster using configuration values from `application.properties`. 
...
The subsequent section has a reference that organizes producer configuration options by concern (batching, reliability, etc.). The **Used in example?** column shows whether the option appears in the sample code.

[IMPORTANT]
====
The rest of this section deals with explaining the various configuration options available for Kafka producers and some of their usages in the code. If you are already familiar with these options or want to directly jump to running the code to save time, feel free to **skip to this section and move on to the link:module-02.2.html[next section]** to run the producer application.
====
...
=== Building a Message/Record (line 60)
...
// === Closing the Producer (line 141-148)
...
. The producer closes and flushes and cleans up.
