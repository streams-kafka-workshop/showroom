...
* From your browser, navigate to {openshift_cluster_console}topology/ns/*globex-{user_name}*?view=graph[OpenShift Cluster Console, window="_console"]* Login with your username and password (`{user_name}`/`{password}`).
...
* Click to navigate to https://amq-streams-console-{user_name}.{openshift_cluster_subdomain}[streams for Apache Kafka console, window="_amqstreams"]* This redirects you to the streams for Apache Kafka console login page* For the purpose of this workshop, choose *Click to login anonymously* to access the console
