...
Kiali offers a number of ways to examine the microservices which are managed by {rhossm}. This helps users to quickly evaluate the health of their service architecture.
...
+
NOTE: Since sidecar injection was enabled just a while ago, it may take a minute or two for  the graph to show the updated metrics.

. You can zoom into the Kiali console to get a better view of the `travel-agency` Traffic Graph.
+
...
. From the Kiali Console, navigate to the http://kiali-istio-system.{ocp_cluster_openshift_cluster_ingress_domain}/console/workloads?duration=60&refresh=10000&namespaces=travel-agency[Workloads^, window="kiali"] menu.
+
...
=== Optional Activity: Detailed metrics exploration

If time permits, you can further explore the metrics offered.

.[underline]#Click to view: Detailed metrics exploration#
