...
* In the browser window, open the tab pointing to the OpenShift console. If you don't have a tab open to the console, click navigate to {openshift_cluster_console}/topology/ns/globex-{user_name}?view=graph[OpenShift console, window="_console"]. If needed login with your username and password (`{user_name}`/`{password}``).
