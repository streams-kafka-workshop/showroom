...
** *class*: the Debezium connector implementation class. We're using PostgreSQL as source database, so the appropriate connector is *io.debezium.connector.postgresql.PostgresConnector*.
...
** *schema.include.list*: the schemas to include in the change data capture process.
...
* Make sure you are on the kafka-workshop folder on the terminal. If you are on the module-kafka-api folder, run the following command to navigate to the kafka-workshop folder.
+
...
+
Filter the topics by *Name*  by the term `globex.updates`. Initially, you will only see one topic, `globex.updates.public.customer`, that will receive the change events.
+
...
=== Up Next
In the next section of the workshop, you will create some orders, and verify that the corresponding change events are picked up by Debezium and published to the above mentioned two topics.  
