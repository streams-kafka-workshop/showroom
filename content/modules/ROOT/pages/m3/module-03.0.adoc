...
With their growing popularity, {corp} would like to add new channels to their existing portals. They plan to launch a mobile app, and also onboard  partner portals which require secure access to their core services.

All inward traffic from mobile app or external systems need to be secured and managed. {corp} also wants to protect the endpoints by easily applying rate limiting and specific levels of access based on systems and users.
...
. *DNS Policy* based on the Gateway configuration, creates dns records in external dns providers for enabling DNS based load balancing across hybrid cloud. It also hellps to manage DNS weighting and geo-based routing of requests.
. *TLS Policy* targets Gateways by managing the lifecycle of tls certificates using using https://cert-manager.io/[CertManager^, window="content"].
...
. The access to core services (Travels) service endpoint needs to be secured and protected for connectivity from the mobile app and partner portals that is being built.
...
. The core services and the sample partner app has been predeployed on your OpenShift cluster.
...
. As part of the activities, you will also setup {rhcl} CRs (custom resources) needed to connect, secure and manage the `travel-agency` service.
