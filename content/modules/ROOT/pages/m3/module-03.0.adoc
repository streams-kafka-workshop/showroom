...
To safely expose their core services to external travel partners, {corp} decides to embrace a policy-as-code approach. This approach provides a repeatable and automated way to manage the policies that can be applied to incoming north-south traffic and there by easily define security through code.
...
{rhcl} extend the capabilities of https://gateway-api.sigs.k8s.io/[*Gateway API*^, window='content'], the new Kubernetes standard for Ingress. The , and offers Policy APIs to support DNS management, TLS certificate lifecycle management, authentication, and rate limiting.
...
Platform Engineers can apply policies to the Gateway and there by govern the the lower levels of the network (HTTPRoute/Services), until a more specific policy is applied.
