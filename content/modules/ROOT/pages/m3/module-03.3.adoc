...
. Click on the *Log in with OpenShift* button, and log in as `admin/{ocp_cluster_openshift_cluster_admin_password}` when prompted.
+
...
. For the sake of this workshop, let us create some traffic to the `echo-api`. Run the following command on the top-right terminal. You will see a combination of 403 and 429 errors in the terminal.
+
...
We will also extend this pattern to include all the important aspects of observability.
