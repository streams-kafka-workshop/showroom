...
. AWS Hosted Zone (DNS domain) `travels.{ocp_cluster_workshop_main_domain}` has been created on *AWS Route 53*.
. A Gateway resource `prod-web` referencing the DNS domain `travels.{ocp_cluster_workshop_main_domain}`
. {rhcl} operator and a few policies to provide dns management in external dns provider
. A sample partner web-portal to test secure external access of the `travel-agency` core  services
...
NOTE: A *DNS Policy* is attached to a Gateway to enable DNS management by managing DNS records in external DNS providers - in this case, *AWS Route53*.  This policy has been pre-created, because DNS resolution for new DNS records may take time to propogate and could delay your progress in this module. 
