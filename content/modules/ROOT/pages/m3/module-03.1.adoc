...
. {rhcl} operator and a few policies to provide dns management in external dns provider are pre-configured.
. A sample partner web-portal to test secure external access of the `travel-agency` core  services has been deployed

== Explore {rhcl} set up

. Navigate to {ocp_cluster_openshift_cluster_console_url}/kuadrant/overview[Connectivity Link Overview^, window='content'], and 
...
+
image:m3/prod-web-snippet.png[link=self, window=blank, width=60%] 
