...
* *Prepare the input stream from the Kafka topic trackingEventTopic for downstream processing (Line 45 - 46)* . This is the entry point of the pipeline
+ 
[source,java]
----
...
* *Transformations and aggregations(Line 47-51)* - This block takes raw user activity messages → parses them → filters irrelevant ones → converts to product scores → groups by product → and finally aggregates likes.
+
[source,java]
----
...
* *Continuously keep track of the Top-N featured products per category by listening to all product score updates, storing and updating the results automatically (Line 53-57)*
+
[source,java]
----
productLikes
