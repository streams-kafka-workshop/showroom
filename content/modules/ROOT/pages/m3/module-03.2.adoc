...
In this section, you will perform the role of a developer. As  self service create policies based on your application's need to protect the core services' endpoints.
...
. You should see a 404 error status code. This is because the Travels core services endpoint hasn't been exposed using a HTTPRoute to make it available to other applications/systems.
+
...
. Next, create a AuthPolicy targetting the HTTPRoute based on your application's need.
...
.. You will notice that the `travel-agency` HTTPRoute now is impacted by it's own AuthPolicy.
.. You may have noticed that the `prod-web` Gateway's RateLimitPolicy still affects the `travel-agency` HTTPRoute.
+
image:m3/travel-ap-topology.png[width=80%] 

[NOTE]
====
For the sake of the workshop, we have chosen to authentication using a simplistic API Key. In real world, you may consider using a JWT/OAuth access token to authenticate the requests.
====
...
. Clicking the  *Find details* button the details more than 5 times.
...
. In this section, you played the role of a Developer and created a `travel-agency` HTTPRoute for the core servcices so that this can be accessed by the partner portal securely. You applied the right Auth and Rate Limit policies to the HTTPRoute there by customising this to your specific needs in a self-service manner
