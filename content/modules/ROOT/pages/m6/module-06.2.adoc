...
We will setup the ACLS for the producer user such that they will be *able to write to the topic `secure-demo` but not create or delete it.* 
...
* We can define the ACLs while creating the KafkaUser resource. Let's create the ACLs for the producer user first. Navigate to the link:{openshift_cluster_console}[OpenShift console, window="console"] and log in with your username and password (`{user_name}`/`{password}`).
...
* The above custom resource creates a KafkaUser with username `producer` and the password is stored in a secret named `producer-password`. Under the authorization section, we define an ACL for the topic `secure-demo`. This user will be able to write to the topic `secure-demo` but not create or delete it. Go ahead and click the create button now
+
image::m6/producer-user-yaml-create.png[]

* Now, let's try to delete the topic using the producer user you just created. 

* Navigate to the *upper terminal* and first create a temporary config file with the producer credentials. 
+
...
* Navigate to the *upper terminal* and run the following command, to see if the producer user can delete the topic `secure-demo`.
+
[source,sh,role="execute",subs=attributes+]
----
...
* The above error is expected and shows the ACL in action as the producer user only has permissions to write to the topic but not delete it. 

* Now, Let's try to write to topic. Run the below command in the *upper terminal* and write some messages to the topic.
+
[source,sh,role="execute",subs=attributes+]
----
...
* Navigate to the *upper terminal* and first create a temporary config file with the consumer credentials. 
+
[source,sh,role="execute",subs=attributes+]
----
cat > /tmp/client.properties <<EOF
...
* In the *upper terminal*, run the following command to see if client with consumer credentials can write to the topic(which the ACL does not allow). 
+
[source,sh,role="execute",subs=attributes+]
----
kafka-console-producer.sh \
...
=== Conclusion
What we have seen so far is a simple example of ACLs. Enterprises can synchronize users and groups from their existing IDMs with *Red Hat build of Keycloak* technologies to implement fine-grained access control. The policies and ACL rules can be configured for the users or groups in Red Hat build of Keycloak which the kafka admins can use to secure the Kafka clusters using ACLs. 

Streams for Apache Kafka supports the use of *OAuth 2.0* token-based authorization through Red Hat build of Keycloak Authorization Services, which allows you to manage security policies and permissions centrally.
