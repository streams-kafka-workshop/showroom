...
=== Creating ACLs
In this section, we will setup ACLs for a producer and a consumer user respectively. We will setup the ACLS for the producer user such that they will be able to write to the topic `secure-demo-again` but not create or delete it. We will setup the ACLs for the consumer user such that they will be able to read from the topic `secure-demo` but not write to it.
