...
[IMPORTANT]
====
If you seen an error message similar to the following:
...
----
then run the following command to make sure you are logged in
[source,bash,role="execute",subs=attributes]
----
{login_command}
----
and then re-run the `oc project kafka-{user_name}` command in the lower terminal.
====
...
If you have skipped module 1 and didn't create the toolbox container earlier, you might see an error message such as *Error from server (NotFound): pods "kafka-toolbox" not found* Create it using the following command in the *upper terminal*.
...
then wait for a few seconds and then connect *`oc rsh -n kafka-{user_name} kafka-toolbox`*
...
* Now let's create the new topic again using the correct credentials. Observe that the credentials are now passed in the form of the config file  and the we are using the secure 9095 listener with authentication.
+
[source,sh,role="execute",subs=attributes+]
----
kafka-topics.sh --bootstrap-server kafka-kafka-bootstrap:9095 \
