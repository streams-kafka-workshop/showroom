...
[IMPORTANT]
====
If you have skipped module 1 and directly moved to module 6, please perform all the below steps to setup your terminals and environment. 

Login to cluster in the *upper terminal*
[source,sh,role="execute",subs=attributes+]
----
{login_command}
----

Create the kafka toolbox using the *upper terminal*
[source,sh,role="execute",subs=attributes]
----
...
Connect to the toolbox using the following command in the *upper terminal*.
...
Login to cluster in the *lower terminal*
[source,sh,role="execute",subs=attributes+]
----
{login_command}
----
...
* Login to the project namespace `kafka-{user_name}` using the following command in the *lower terminal.* 
+
...
* *Port 9095 – SASL/SCRAM Listener: Authenticated client access.*
...
* Make sure you are still connected to the Kafka toolbox container we created in the previous modules.
+
image::m6/tool-box-shell.png[]
+
[IMPORTANT]
====
If not connected to the toolbox container, connect to it again using the following command in the *upper terminal*.
...
* In the **upper terminal**, create a new topic `secure-demo` using the secure 9095 listener, without authentication. A similar command to the one we used in the previous modules but targeting the authenticated 9095 listener instead of 9092. Let's see what happens.
+
[source,sh,role="execute",subs=attributes+]
----
...
* Wait for a minute or so and observe the topic creation times out and the command fails. You should see the following error message:
+
[source,bash]
----
...
* We've already created an admin user on this cluster with all the privileges needed. We will use this user to create the topic using the secure listener with authentication. 

* First, let's create a local config file with the necessary authentication details. Run the following command in the *upper terminal*.
+
[source,sh,role="execute",subs=attributes+]
----
...
* Now let's create the new topic again using the correct credentials. Observe that the credentials are now passed in the form of the config file  and we are using the secure 9095 listener with authentication.
+
[source,sh,role="execute",subs=attributes+]
----
kafka-topics.sh --bootstrap-server kafka-kafka-bootstrap:9095 \
