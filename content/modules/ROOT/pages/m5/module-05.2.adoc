...
* Navigate to the link:https://devspaces.{openshift_cluster_subdomain}[Devspaces IDE, window="devspaces"]

* Login with your credentials - `{user_name}` and `{password}` if needed.

* From the Devspaces dashboard, click on *open* button to launch your workspace.
+
image::m2/devspaces-launch.png[]

* Open the file *module-registry/src/main/resources/application.properties* in the file explorer
+
...
* The config file will look like the following. Observe that along with the registry API URL and bootstrap servers, we have also defined the group id, artifact id, version topic name and consumer group information needed in this file.
+
...
* Make sure the terminal is configured to use Java 21.
+
[source,sh,role="execute",subs=attributes+]
----
...
* Create a topic with the name `schema-demo` using the following command. Recall that we used the same name as the artifact id while creating the schema in the previous section. 
+
[source,bash,role="execute",subs=attributes+]
----
...
* You should see an output similar to the following, indicating that the schema was retrieved successfully from the Schema Registry.You should see the same schema that we added in the previous section.
+
...
* Paste this message into the terminal and *press enter twice*. This json is valid and conforms to the schema defined in the schema registry, which means this must be accepted and sent to the topic `schema-demo`.
+
...
✓ Message sent successfully!
   ...
   Schema source: Registry (artifact 'schema-demo' in group 'my-group')
...
* Paste this message into the terminal and *press enter twice*. This json is invalid because our schema  indicates the type of `customer_id` to be `string` and this provides an integer value. This message should not be accepted.
+
[source,json,role="execute",subs=attributes+]
----
{
...
* Navigate to link:https://service-registry-ui-{user_name}.{openshift_cluster_subdomain}/explore/my-group/schema-demo[service registry UI, window="_schema-registry"] > my-group > schema-demo
...
* We are going to paste a new version of the schema that introduces a new field called `phone` in the existing schema and makes it a required field. This breaks backward compatibility and should trigger the compatibility rule in the registry. Let's see that in action.
...
This brings us to the end of this module. In order to understand more about the different compatibility rules, you can refer to the link:https://docs.redhat.com/en/documentation/red_hat_build_of_apicurio_registry/3.1/html/apicurio_registry_user_guide/registry-rule-reference_registry#registry-rule-types_registry[documentation here, window="docs"].
