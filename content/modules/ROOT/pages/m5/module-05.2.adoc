...
== Producer and Consumer with Schema Registry
...
* Open the file *src/main/java/com/example/Config.java* in the file explorer
+
image::config-java.png[]
...
* The config file will look like the following. Observe that along with the registry API URL, we have also defined the group id, artifact id, version, topic name and bootstrap servers information needed in this file.
+
image::config-java-variables.png[]
...
+
image::retrieve-schema-output.png[]
...
* You should see an output similar to the following, indicating that the message was sent successfully.
+
[source,bash]
----
...
* Navigate to link:https://service-registry-ui-{user_name}.{openshift_cluster_subdomain}/xplore/my-group/schema-demo[service registry UI, window="_schema-registry"].

* Navigate to the rules tab and click on the *Enable* button in the *Compatibility rule* row. 
+
image::enable-rule.png[]
...
* Click on the Overview tab and click on the Crate version button.
+
image::create-version.png[] 
...
* Click on the *Create Version* button again and paste the below schema, which is the same the one we provided earlier but provides a default value for the `phone` field in case it's missing. This time the compatibility rule should not trigger and new version of the schema should be created.
+
[source,avro,role="execute",subs=attributes+]
----
{
...
* Now producers can send messages with or without the `phone` field and the messages will still get accepted. You can navigate back to dev spaces terminal and run the below command to start the application
+
...
* This message with a valid `phone` field is accepted and sent to the topic `schema-demo` too. This shows the backward compatibility feature of the schema registry.

* Now choose option 3 to run the consumer and observe the new messages. For the messages that we have not sent with the `phone` field, the consumer will use the default value of 0 for the `phone` field.
