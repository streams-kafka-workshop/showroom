...
The dynamic assignment of partitions to consumers within a group is managed through a process called *rebalancing*. Rebalances occur when a consumer joins or leaves a group, crashes, or when a subscribed topic's partitions are modified.
...
* Now let's start three consumers in the same consumer group, *demo-cg*, one by one, using a simple bash script. As you proceed through the script and keep starting new consumers, you will see the rebalancing of partitions between the consumers. 

* Exit the shell in the *lower terminal* if you are already inside the toolbox container.
+
...
* Navigate to the https://amq-streams-console-{user_name}.{openshift_cluster_subdomain}/kafka[streams console, window="_amqstreams"] > consumer groups > demo-cg to view the first consumers and partitions being assigned to each consumer. 
+
...
* Go back to the https://amq-streams-console-{user_name}.{openshift_cluster_subdomain}/kafka[streams console, window="_amqstreams"] and observe the partitions being assigned to cg-c1 and cg-c2.  You will see a rebalance happening and partitions getting reassigned between cg-c1 and cg-c2 as shown below. 
...
* Go back to the https://amq-streams-console-{user_name}.{openshift_cluster_subdomain}/kafka[streams console, window="_amqstreams"] and observe the partitions being assigned to cg-c1, cg-c2 and cg-c3.
+
image::m1/demo-cg-c1-c2-c3.png[]


* Go ahead and start the 4th consumer by pressing enter in the *lower terminal*. Do not press enter again to stop the other consumers yet.

* You will see that the 4th consumer (cg-c4) is started but it is not assigned any partitions and remains idle as shown below. This is because, we have 3 partitions and 3 consumers. Since there are no more partitions left, the 4th consumer will not be assigned any partitions. Kafka ensures that each partition is consumed by only one member of a consumer group at any given time to avoid duplicate processing of messages. 
+
image::m1/demo-cg-c1-c2-c3-c4.png[]

=== Offsets and Lag
*Offsets:* Every message gets a unique, sequential number inside its partition. This "page number" lets the consumer mark exactly where it stopped reading.

*Lag:* Lag measures how far behind a consumer is.

Example: If the broker is writing message #100, but the consumer is currently reading message #90, the Lag is 10.

High lag means your application is slow and struggling to keep up with the incoming data.

image::m1/roundup-image.png[]



* Navigate to the https://amq-streams-console-{user_name}.{openshift_cluster_subdomain}/kafka[streams console, window="_amqstreams"] > topics > consumer groups > demo-cg and expand consumer cg-c1 we started previously. 
