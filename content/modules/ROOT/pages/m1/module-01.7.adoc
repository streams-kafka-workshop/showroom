...
For each partition, Kafka maintains multiple copies called replicas, distributed across different Kafka brokers:
...
** When a broker with a leader partition goes down, a new leader is elected on a different node.
...
* In the **upper terminal**, create a new topic with replication factor of 3
+
...
* If the leader broker goes down, one of the followers will be automatically elected as the new leader to ensure high availability. This ensures that producers and consumers can continue to operate without interruption.
