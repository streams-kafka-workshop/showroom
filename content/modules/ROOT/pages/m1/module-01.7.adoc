...
[source,sh,role="execute",subs=attributes+]
----
oc run -n kafka-{user_name} topic-describe-rf3 --rm -it --restart=Never \
  --image=registry.redhat.io/amq-streams/kafka-40-rhel9:3.0.0 -- \
  bash -lc '/opt/kafka/bin/kafka-topics.sh --bootstrap-server kafka-kafka-bootstrap:9092 --describe --topic replicated'
----
+
