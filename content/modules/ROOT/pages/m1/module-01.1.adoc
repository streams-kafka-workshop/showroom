...
. In the upper terminal, create a Kafka topic named `demo` with 3 partitions and a replication factor of 1 (we will learn about replication later) using the following command.
+
...
. Click https://amq-streams-console-{user_name}.{openshift_cluster_domain}/kafka/E71SE2wtRUOMJFtaf9Mviw/topics/AcbEWuRbQR613oZf8RLm_g/messages[here, window="console"] to navigate to the streams console to view the topic you created. The streams console is a web-based user interface for visualizing and monitoring Kafka clusters, topics, messages etc.
