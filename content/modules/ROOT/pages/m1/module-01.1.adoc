...
+
You will also notice a `Ready` status displayed in the *Details* tab
+
image:m1/site-ready.png[] 
. Once the site creation is complete, from the *Get Started tab* click on the *Deploy Network Console* button. 
+
...
+
_Click to view the image in full screen._
+
image::m1/deploy_console_rhsi.png[link=self, window=blank, width=100%]
...
. Click on *Create listener* to set up a listener in the `travel-db` namespace.
+
...
.. If you would like to inspect how the token looks, you can download and open the file. 
.. Click on *Done*. 
+
