...
. You will get a `{"error":"Travel Quote for Prague not found"}` message indicating that the database is not accessible and the service is unable to get a travel quote for Prague. 
+
Let's set up the Virtual Application Network using Red Hat Service Interconnect in the subsequent steps to help the core services (on ) access the database on-premises.
...
. Navigate to Home -> Projects -> travel-db, if you are not in the `travel-db` project now.
+
...
****
. In the *Create site* wizard, keep the default values. Note that the *Enable link access* is checked, indicating that this site will allow ingress connections from other sites.  +
