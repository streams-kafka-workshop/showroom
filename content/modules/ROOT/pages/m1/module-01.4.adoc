...
* A consumer can read data from any offset within a topic partition. In most cases, a consumer advances its offset linearly, but it can seek to any offset and read sequentially from that point.
...
* Connect to the toolbox container you created by running the following command in the *lower terminal*.
+
[source,sh,role="execute",subs=attributes+]
----
