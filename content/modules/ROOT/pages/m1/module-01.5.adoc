...
As we learned earlier, With a key, the records/messages with the same key go to the same partition. Let's see this in action.

* In the *upper terminal* run a new producer that will send a bunch of messages with key-value pair . This will ensure messages with the same key go to the same topic partition.
+
...
* Wait for *10 seconds* after the producer starts to allow some messages to be produced. The messages produced will be in the format `key:value` such as `k1:msg-1`, `k2:msg-2` etc

NOTE: You will not see any output in the terminal since the console producer does not print any confirmation messages when messages are produced successfully. Just wait for 10 seconds before proceeding to the next step.

* Now stop the producer by hitting (Ctrl+C).
