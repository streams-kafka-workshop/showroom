...
They were successful in breaking down all their components into microservices and migrated all their components to containers and deployed them on Red Hat OpenShift. However, their core database remained in the data center, mainly because there are regulatory requirements that require data processing or storage to happen on-prem or in specifically certified data centers. The company didn't want to move their database to the cloud yet and want to keep it on-premises on their Red Hat Enterprise Linux clusters.
