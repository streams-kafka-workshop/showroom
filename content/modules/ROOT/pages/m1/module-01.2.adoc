...
* Ordering is guaranteed within a single partition. To ensure that related messages end up in the same partition (and are therefore ordered relative to each other), send them with the same key.
...
=== Up Next
This concludes a very high-level overview of how Kafka partitions work. In the next section, we'll learn about Kafka producers and consumers.