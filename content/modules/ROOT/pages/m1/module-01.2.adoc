...
Similar to the OpenShift site, you will setup the site and routers on RHEL using the {rhsi} command line interface.

. You need  to ssh into the on-prem RHEL machine using the in-built Terminal to the right. 
...
. In the *RHEL terminal window (lower terminal)*, run the following command to create a site. Similar to the OpenShift site, this will setup the routers in the namespace.
+
[source,sh,role="execute",subs=attributes+]
----
...
. Click on *Login with OpenShift*, and enter the username `admin` and password `{ocp_cluster_openshift_cluster_admin_password}` if prompted.
