...
AMQ Streams is Kafka built for OpenShift/Kubernetes, using the Strimzi operators to make Kafka declarative and automated:
...
This workshop introduces you to the fundamentals of Event Streaming with Apache Kafka. You will learn key concepts such as topics, partitions, producers, consumers, consumer groups, and replication. You will also gain hands-on experience by using a sample application to understand these concepts.
...
Before we get started with Kafka,let's first understand the concept of Event Streaming and why Kafka is the most widely used event-streaming platform.